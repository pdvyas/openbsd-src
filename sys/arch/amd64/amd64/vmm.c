	...
	while (ret == 0) {
		...
		if (irq != 0xFFFF) {
			if (vcpu->vc_irqready) {
				...
				vmcb->v_irq = 3;
				vmcb->v_intr_misc = 0x1f; /* XXX #define ign_tpr */
				vmcb->v_intr_vector = irq;
		...
		if (ret == 0) {
			...
			if (vmcb->v_intr_shadow) {
				printf("shadow!\n");
			}

			if ((vcpu->vc_gueststate.vg_rflags & PSL_I) || vmcb->v_intr_shadow)
				vcpu->vc_irqready = 1;
			else
				vcpu->vc_irqready = 0;
